...
export default class ImageShaper {
    ...
    public async Resize(imageName:string, height:number, width:number): Promise<string>{
        let imageLocation = path.join(config.uploads,imageName);
        const thumbFilePath = path.join(config.thumbnails, `${height}_${width}`+imageName)
