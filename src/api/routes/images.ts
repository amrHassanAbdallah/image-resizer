...
export default (app: Router) => {
    ...
    route.get('/:imageName', async (req: Request, res,next) => {
        ...
        if (req.query.width || req.query.height){
            if (isNaN(parseInt(<string>req.query.width)) || isNaN(parseInt(<string>req.query.height)) ){
                return res.status(400).json({
                    "message": "invalid value for width or height, both required & must be integer"
                })
        ...
        if (req.query.height) {
            try {
                ...
                imageLocation = await imageShaper.Resize(imageName,parseInt(<string>req.query.height),parseInt(<string>req.query.width))
            } catch (e) {
